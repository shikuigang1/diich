...
public class ContentFragmentServiceImpl extends BaseService<ContentFragment> implements ContentFragmentService  {
    ...
    private void checkSaveField(ContentFragment contentFragment) throws Exception {
        ...
        if (attributeId != null && attributeId != 0) {
            ...
            attribute.setTargetType(contentFragment.getTargetType());
            ...
            for (Attribute  attr : attributeList) {
                if(attr.getCnName().equals(contentFragment.getAttribute().getCnName())){
                    throw new ApplicationException(ApplicationException.PARAM_ERROR, attribute.getCnName().toString()+" 字段已存在");
                }
            }
        }
    }
