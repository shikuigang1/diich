...
var loginPage = {
    ...
    template: function () {
        var html = '<style>'+
            ...
            '                <div class="name">&nbsp;</div>'+
            '                 <div class="area">'+
            '                    <label for="policy"><input id="policy" type="checkbox" checked="">我已仔细阅读并接受<a target="_blank" href="http://diich.com/page/copyright.html">《版权声明》</a>及<a target="_blank" href="http://diich.com/page/privacy.html">《隐私保护政策》</a></label>'+
            '                </div>'+
            '            </div>'+
            ...
            if(data.code==3){
                ...
                $.grep(filterpage, function(val, key) {
                    if (path.indexOf(filterpage[key]) != -1) {
                        ...
                        //$('.header_detail .content .info li em').click();
                        return false;
                    }
                });
            }
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if(data.code!=0){
                ...
                if(path.indexOf("/usercenter.html") != -1){
                    //加载数据
...
function loginedTemplate(data) {
    var str='<div class="main">' +
        ...
        '      </div>' +
        '      <div class="item account"><a href="">个人中心</a></div>' +
